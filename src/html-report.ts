...
.criteria-toolbar{position:sticky;top:72px;z-index:8;display:flex;gap:8px;padding:12px 14px;align-items:center;flex-wrap:wrap;margin-bottom:12px;box-shadow:0 10px 30px color-mix(in srgb,var(--bg) 75%,transparent)}.filter{border:1px solid var(--line);border-radius:999px;background:var(--surface-2);color:var(--muted);padding:6px 12px;font-size:11px;cursor:pointer}.filter.on{color:var(--accent);border-color:var(--accent);background:var(--accent-soft)}.criteria-legend{display:flex;gap:12px;margin-left:8px;color:var(--muted);font:10px var(--mono);text-transform:uppercase}.legend-item{display:flex;align-items:center;gap:5px}.legend-dot,.difficulty-dot{width:7px;height:7px;border-radius:50%;background:currentColor}.legend-item.met{color:var(--ok)}.legend-item.unmet{color:var(--accent)}.legend-item.na{color:var(--skip)}.search{margin-left:auto;min-width:280px;border:1px solid var(--line);border-radius:7px;background:var(--surface-2);color:var(--ink);padding:8px 10px;outline:none}.search:focus{border-color:var(--accent)}
.criteria-groups{display:flex;flex-direction:column;gap:14px}.criterion-category{padding:20px}.category-head{display:flex;align-items:center;gap:14px}.category-id{color:var(--accent-2);font:11px var(--mono)}.category-title{font-size:16px;font-weight:650;white-space:nowrap}.category-rule{height:1px;flex:1;border-top:1px dashed var(--line-strong)}.category-score{font:700 16px var(--mono)}.category-copy{display:flex;justify-content:space-between;gap:18px;margin:7px 0 17px;color:var(--muted);font-size:11px}.category-counts{white-space:nowrap;font-family:var(--mono)}.criterion-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(164px,1fr));gap:9px}.criterion-card{min-height:154px;border:1px solid transparent;border-radius:7px;background:var(--surface-2);padding:13px;text-align:left;cursor:pointer;display:flex;flex-direction:column;position:relative;overflow:hidden}.criterion-card:hover{border-color:var(--accent);transform:translateY(-1px)}.criterion-card.fail{background:linear-gradient(145deg,color-mix(in srgb,var(--accent) 42%,var(--surface)),color-mix(in srgb,var(--accent) 28%,var(--surface)))}.criterion-card.pass{border-color:color-mix(in srgb,var(--ok) 13%,var(--line))}.criterion-card.skip{opacity:.48}.criterion-card:focus-visible{outline:2px solid var(--accent);outline-offset:2px}.criterion-top{display:flex;align-items:center;gap:7px;font:10px var(--mono);letter-spacing:.03em;text-transform:uppercase}.difficulty-dot.basic{color:var(--warn)}.difficulty-dot.intermediate{color:var(--accent)}.difficulty-dot.advanced{color:var(--bad)}.criterion-state{margin-left:auto;color:var(--muted);font-size:9px}.criterion-card.fail .criterion-state{color:var(--ink)}.criterion-name{font-size:15px;font-weight:600;line-height:1.3;margin:12px 0;overflow-wrap:anywhere}.criterion-meta{color:color-mix(in srgb,var(--muted) 80%,transparent);font:9px var(--mono);margin-top:auto;padding-top:10px}.criterion-score{align-self:flex-end;font:13px var(--mono);margin-top:7px}.criterion-card.fail .criterion-score{color:var(--ink)}.criterion-card.pass .criterion-score{color:var(--ok)}.criterion-card.skip .criterion-score{color:var(--skip)}.status:before{content:"";width:7px;height:7px;border-radius:50%;background:currentColor}.status.pass{color:var(--ok);background:var(--ok-soft)}.status.fail{color:var(--bad);background:var(--bad-soft)}.status.skip{color:var(--skip);background:var(--skip-soft)}.empty{padding:40px;color:var(--muted);text-align:center}
...
@media(max-width:1050px){.visual-grid{grid-template-columns:1fr}.fix-grid{grid-template-columns:1fr 1fr}.pillar-grid{grid-template-columns:repeat(3,1fr)}.summary-grid{grid-template-columns:1fr 1fr}.levels{grid-template-columns:1fr}.criterion-grid{grid-template-columns:repeat(auto-fill,minmax(150px,1fr))}.navlinks{display:none}}
@media(max-width:680px){.shell,.topbar-inner{padding-left:14px;padding-right:14px}.overview{padding:16px}.overview-actions{width:100%;margin:8px 0 0}.segments{gap:1px}.summary-grid{margin:18px -16px -16px}.fix-grid,.pillar-grid,.detail-grid{grid-template-columns:1fr}.visual-grid{grid-template-columns:minmax(0,1fr)}.chart-panel{min-height:320px}.radar-chart,.trend-chart{height:auto}.criteria-toolbar{position:static}.criteria-legend{order:3;margin-left:0;width:100%}.search{min-width:100%;margin-left:0;order:4}.criterion-category{padding:15px}.category-head{gap:8px}.category-copy{display:block}.category-counts{display:block;margin-top:6px}.criterion-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.criterion-card{min-height:140px}.score-marker{display:none}}
@media print{.topbar,.overview-actions,.criteria-toolbar,.text-btn,dialog{display:none!important}.panel,.fix-card,.pillar-card,.criterion-card{break-inside:avoid}body{background:#fff}.shell{max-width:none}}
</style>
</head>
<body>
...
<section id="criteria"><div class="section-head"><div><div class="eyebrow">Full assessment</div><h2>Criteria by category</h2><p>See what is met, what needs work, and what is not applicable. Open any card for rationale, evidence, and an agent remediation prompt.</p></div></div></div><div class="panel criteria-toolbar"><button class="filter on" data-filter="all">All</button><button class="filter" data-filter="fail">Not met</button><button class="filter" data-filter="pass">Met</button><button class="filter" data-filter="skip">N/A</button><div class="criteria-legend" aria-hidden="true"><span class="legend-item unmet"><i class="legend-dot"></i>Not met</span><span class="legend-item met"><i class="legend-dot"></i>Met</span><span class="legend-item na"><i class="legend-dot"></i>N/A</span></div><input id="search" class="search" type="search" placeholder="Search criterion, category, id, or app…"></div><div class="criteria-groups" id="criteria-groups"></div><div class="panel empty" id="criteria-empty" hidden>No criteria match this filter.</div></section>
...
function renderCriteria(){var q=query.toLowerCase();var items=D.findings.map(function(f,index){return {f:f,index:index}}).filter(function(item){var f=item.f,st=statusOf(f);if(filter!=='all'&&st!==filter)return false;if(pillarFilter&&f.pillar!==pillarFilter)return false;var pillar=D.pillars[f.pillar];return !q||(f.name+' '+f.id+' '+f.pillar+' '+(pillar?pillar.name:'')+' '+(f.app||'')).toLowerCase().indexOf(q)>=0});var rank={fail:0,pass:1,skip:2};var groups=Object.keys(D.pillars).map(function(id){var categoryItems=items.filter(function(item){return item.f.pillar===id}).sort(function(a,b){return rank[statusOf(a.f)]-rank[statusOf(b.f)]});return {id:id,items:categoryItems}}).filter(function(group){return group.items.length});$('criteria-groups').innerHTML=groups.map(function(group){var pillar=D.pillars[group.id];var all=D.findings.filter(function(f){return f.pillar===group.id});var met=all.filter(function(f){return f.pass&&!f.skipped}).length,unmet=all.filter(function(f){return !f.pass&&!f.skipped}).length,na=all.filter(function(f){return f.skipped}).length;var cards=group.items.map(function(item){var f=item.f,st=statusOf(f),state=st==='pass'?'met':st==='fail'?'not met':'N/A',score=st==='pass'?'1/1':st==='fail'?'0/1':'N/A',difficulty=(f.difficulty||'intermediate').toLowerCase();return '<button class="criterion-card '+st+'" data-index="'+item.index+'" aria-label="'+escapeHtml(f.name+', '+state)+'"><div class="criterion-top"><i class="difficulty-dot '+escapeHtml(difficulty)+'"></i><span>'+escapeHtml(difficulty)+'</span><span class="criterion-state">'+state+'</span></div><div class="criterion-name">'+escapeHtml(f.name)+'</div><div class="criterion-meta">'+escapeHtml(f.id)+(f.app?' · '+escapeHtml(f.app):'')+(f.droidLevel?' · L'+f.droidLevel:'')+'</div><div class="criterion-score">'+score+'</div></button>'}).join('');return '<article class="panel criterion-category" data-category="'+escapeHtml(group.id)+'"><div class="category-head"><span class="category-id">'+escapeHtml(group.id)+'</span><span class="category-title">'+escapeHtml(pillar.name)+'</span><span class="category-rule"></span><span class="category-score">'+pillar.pct.toFixed(1)+'%</span></div><div class="category-copy"><span>'+escapeHtml(pillar.rationale)+'</span><span class="category-counts">'+met+' met · '+unmet+' not met'+(na?' · '+na+' N/A':'')+'</span></div><div class="criterion-grid">'+cards+'</div></article>'}).join('');$('criteria-empty').hidden=items.length>0;document.querySelectorAll('.criterion-card').forEach(function(card){card.addEventListener('click',function(){openDetail(Number(card.dataset.index))})})}
