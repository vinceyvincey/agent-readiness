...
export const CRITERIA_REGISTRY: CriterionDef[] = [
  {
    droidId: 'large_file_detection',
    piId: 'P5.12',
  ...
  },
  {
    ...
    skippable: false,
    description:
      'Tooling tracks technical debt markers. Common approaches: TODO/FIXME scanner in CI, TODO comments required to link to issues (e.g., TODO(TICKET-123) enforcement), language-specific linter rules (eslint-plugin-no-unsanitized-todo, pylint fixme), SonarQube/SonarCloud (has built-in technical debt tracking via SQALE methodology enabled by default; verify it is not explicitly disabled in sonar properties). Other tech debt tracking tools, code quality platforms, or documented tracking systems also sat',
    evaluation:
  ...
  {
    droidId: 'build_cmd_doc',
    piId: 'P3.2',
    name: 'Build command documented',
    level: 2,
    scope: 'repo',
    skippable: false,
    description: 'README/AGENTS.md lists "npm run build" (TS) or "pip install -r requirements.txt" (Py)',
    evaluation:
  ...
  },
  {
    ...
    skippable: false,
    description:
      'lockfile committed (package-lock.json, yarn.lock, pnpm-lock.yaml) for TS; requirements.txt with == pins or poetry.lock for Py',
    evaluation:
  ...
  },
  {
    ...
    skippable: true,
    description:
      'Check for automation that generates code review comments on PRs. If `gh` or `glab` CLI is available and authenticated, run `gh pr list --state all --limit 10 --json reviews,comments` to verify bots/automation are posting review comments (not just status checks). Look for danger.js, droid exec reviews, custom GitHub Actions comments, or AI-powered review bots. Key is automation that GENERATES review content, not just runs checks. Skip if `gh`/`glab` CLI is not available or not authenticated.',
    evaluation:
      'Automated PR review generation – Check for automation that generates code review comments on PRs. If `gh` or `glab` CLI is available and authenticated, run `gh pr list --state all --limit 10 --json reviews,comments` to verify bots/automation are posting review comments (not just status checks). Look for danger.js, droid exec reviews, custom GitHub Actions comments, or AI-powered review bots. Key is automation that GENERATES review content, not just runs checks. Skip if `gh`/`glab` CLI is not available or not authenticated.',
  },
  {
    droidId: 'agentic_development',
    piId: 'P1.6',
    name: 'Agentic development detected',
    level: 3,
    scope: 'repo',
    skippable: false,
    description:
      "Look for evidence that AI agents are part of the development workflow. Check: 1) Git history for agent co-authorship: `git log --format='%an|||%ae|||%s|||%b' -100` and search for AI coding agent identifiers in author/co-author fields. Common patterns include AI tool names (often with '[bot]' suffix) in author fields or 'Co-authored-by' headers (e.g., 'factory-droid[bot]', 'Claude Code'). Note: dependency bots like dependabot or renovate do not count. Also note that these examples are non-exhaust",
    evaluation:
  ...
  },
  {
    ...
    skippable: true,
    description:
      'System deploys multiple times per week with automation. If `gh` or `glab` CLI is available and authenticated, run BOTH: 1) `gh release list --limit 30` to check for release-based deploys. 2) For workflow-based deploys, first list workflows with `ls .github/workflows/ | grep -i deploy` to find deploy workflow filenames, then run `gh run list --workflow={exact-name}.yml --limit 30` for each (gh CLI does not support wildcards in --workflow). Alternatively, run `gh run list --limit 50` and filter fo',
    evaluation:
      'Frequent deployments – System deploys multiple times per week with automation. If `gh` or `glab` CLI is available and authenticated, run BOTH: 1) `gh release list --limit 30` to check for release-based deploys. 2) For workflow-based deploys, first list workflows with `ls .github/workflows/ | grep -i deploy` to find deploy workflow filenames, then run `gh run list --workflow={exact-name}.yml --limit 30` for each (gh CLI does not support wildcards in --workflow). Alternatively, run `gh run list --limit 50` and filter for deploy-related workflows. Some orgs use releases, others use workflow runs ',
  },
  {
    droidId: 'single_command_setup',
    piId: 'P8.2',
    ...
    skippable: false,
    description:
      "README or AGENTS.md or SKILLS documents a single command (or short sequence) that takes you from fresh clone to running dev server. Example: 'npm install && npm run dev' or 'make dev'.",
    evaluation:
  ...
  {
    droidId: 'feature_flag_infrastructure',
    piId: 'P4.14',
    ...
    skippable: false,
    description:
      'LaunchDarkly, Statsig, Unleash, GrowthBook, or custom feature flag system is configured. Enables agents to ship changes behind toggles, reducing risk of agent-authored code affecting all users immediately.',
    evaluation:
  ...
  {
    droidId: 'release_notes_automation',
    piId: 'P4.15',
    ...
    skippable: false,
    description:
      'Automated release notes or changelog generation exists. Does not need to run on every commit - can be periodic (weekly/release-based) via semantic-release, standard-version, changesets, GitHub releases, or custom scripts. Ensures agent contributions are documented.',
    evaluation:
  ...
  },
  {
    ...
    skippable: true,
    description:
      'Canary deployments, percentage-based rollouts, or ring deployments are configured. Allows agent-shipped changes to reach a small percentage of users first, catching issues before full rollout. Skip if not an infra repo.',
    evaluation:
  ...
  },
  {
    ...
    skippable: true,
    description:
      'One-click or automated rollback capability exists and is documented. If an agent ships a bad change, the system can quickly revert without manual intervention or deep investigation. Skip if not an infra based repo.',
    evaluation:
  ...
  {
    droidId: 'version_drift_detection',
    piId: 'P3.9',
  ...
  },
  {
    droidId: 'agents_md',
    piId: 'P1.1',
    name: 'AGENTS.md exists at repo root',
    level: 2,
    scope: 'repo',
    skippable: false,
    description:
      'Check for AGENTS.md file in repository root directory. File should document essentials for autonomous agents like: npm/bun/yarn scripts (TS/JS), pip/venv/poetry setup (Python), build commands, test commands, development workflow, and project-specific conventions. Verify file exists and is not empty (>100 characters). See https://docs.factory.ai/factory-docs/agents-md for reference.',
    evaluation:
  ...
  },
  {
    droidId: 'readme',
    piId: 'P0.1',
    name: 'README.md exists at repo root with setup/usage instructions.',
    level: 1,
    scope: 'repo',
    skippable: false,
    description: 'README.md exists at repo root with setup/usage instructions.',
    evaluation: 'README.md exists at repo root with setup/usage instructions.',
  },
  {
    droidId: 'automated_doc_generation',
    piId: 'P0.8',
    ...
    skippable: false,
    description:
      'Tools/workflows that create/update docs. Examples: API doc generators (Swagger/OpenAPI), code comment extractors (JSDoc, Sphinx), architecture diagram generators, droid exec creating docs, changelog generators, or README updaters. Must show evidence of automated doc creation, not just static docs.',
    evaluation:
  ...
  {
    droidId: 'documentation_freshness',
    piId: 'P0.7',
    ...
    skippable: false,
    description:
      'Run `git log --since="180 days ago" --name-only -- README.md AGENTS.md CONTRIBUTING.md | grep -E "\.(md)$" | head -1`. PASS if at least one of README.md, AGENTS.md, or CONTRIBUTING.md was modified in the last 180 days. This is a simple binary check: key docs updated recently = pass.',
    evaluation:
  ...
  {
    droidId: 'service_flow_documented',
    piId: 'P0.3',
  ...
  },
  {
    ...
    skippable: false,
    description:
      'Check for automation that validates AGENTS.md stays consistent with code. Look for: 1) CI job that checks AGENTS.md commands still work. 2) Automated AGENTS.md generation/update (droid that updates it). 3) Pre-commit hook validating AGENTS.md commands. 4) Documentation testing (running commands from docs). 5) Link checker for AGENTS.md references. PASS if any validation of AGENTS.md accuracy exists. PREREQUISITE: agents_md must pass.',
    evaluation:
  ...
  {
    droidId: 'devcontainer',
    piId: 'P8.3',
    name: 'Dev container configured',
    level: 2,
    scope: 'repo',
    skippable: false,
    description:
      '.devcontainer/devcontainer.json with Node.js & TS extensions (TS) or Python image with pip/poetry (Py)',
    evaluation:
      'Dev container configured – .devcontainer/devcontainer.json with Node.js & TS extensions (TS) or Python image with pip/poetry (Py)',
  },
  {
    droidId: 'env_template',
    piId: 'P8.1',
    ...
    skippable: false,
    description:
      '.env.example file exists or environment variables are documented in README/AGENTS.md. Without knowing required env vars, agents cannot run the application locally. Absolute blocker.',
    evaluation:
  ...
  },
  {
    ...
    skippable: true,
    description:
      'docker-compose.yml for local dependencies (Postgres, Redis, etc.) or clear documentation on how to run them. Agents need these services to run integration tests and develop features. Skip for apps without external service dependencies.',
    evaluation:
  ...
  },
  {
    ...
    skippable: true,
    description:
      'The devcontainer can be built and run successfully using the devcontainer CLI or VS Code. Validates that the containerized development environment actually works, not just that config files exist. Skip if devcontainer CLI is not installed.',
    evaluation:
  ...
  },
  {
    ...
    skippable: false,
    description:
      'Look for external pointers to runbooks/playbooks (links to Notion, Confluence, internal wiki, or dedicated runbooks/ directory). Check README, AGENTS.md, or docs/ for references to incident response procedures. PASS if any documentation points to runbooks, even if hosted externally.',
    evaluation:
  ...
  {
    droidId: 'branch_protection',
    piId: 'P6.6',
    name: 'Branch protection rules enforced',
    level: 2,
    scope: 'repo',
    skippable: true,
    description:
    ...
    evaluation:
      "Branch protection rules enforced – If `gh` or `glab` CLI is available and authenticated, first check admin access: GitHub: `gh api repos/{owner}/{repo} --jq '.permissions.admin'`, GitLab: `glab api projects/{id} --jq '.permissions.project_access.access_level'` (need >= 40). If no admin/maintainer access, skip this criterion. If access confirmed, check in order: 1) Modern rulesets: run `gh api repos/{owner}/{repo}/rulesets` and look for active rulesets targeting main/dev branches. If found, inspect ruleset details with `gh api repos/{owner}/{repo}/rulesets/{id}` to verify PR review requirements",
  },
  {
    droidId: 'secret_scanning',
    piId: 'P6.4',
    ...
    skippable: true,
    description:
      "Repository scans for committed secrets. If `gh` or `glab` CLI is available and authenticated, first check admin access: GitHub: `gh api repos/{owner}/{repo} --jq '.permissions.admin'`, GitLab: `glab api projects/{id} --jq '.permissions.project_access.access_level'` (need >= 40). If no admin/maintainer access, skip the native secret scanning API check but still check for other approaches. Native check: run `gh api /repos/{owner}/{repo}/secret-scanning/alerts`; 404 with \"disabled\" message = FAIL (",
    evaluation:
  ...
  },
  {
    ...
    scope: 'repo',
    skippable: false,
    description: 'in root or .github/ directory with valid team assignments',
    evaluation: 'CODEOWNERS file exists – in root or .github/ directory with valid team assignments',
  },
  {
    droidId: 'automated_security_review',
    piId: 'P6.7',
    ...
    skippable: true,
    description:
      "System automatically generates security review reports or assessments. If `gh` or `glab` CLI is available and authenticated, first check admin access: GitHub: `gh api repos/{owner}/{repo} --jq '.permissions.admin'`, GitLab: `glab api projects/{id} --jq '.permissions.project_access.access_level'` (need >= 40). If no admin/maintainer access, skip the code-scanning API check but still check for other approaches. Code scanning check: run `gh api /repos/{owner}/{repo}/code-scanning/alerts` for SAST t",
    evaluation:
      "Automated security review generation – System automatically generates security review reports or assessments. If `gh` or `glab` CLI is available and authenticated, first check admin access: GitHub: `gh api repos/{owner}/{repo} --jq '.permissions.admin'`, GitLab: `glab api projects/{id} --jq '.permissions.project_access.access_level'` (need >= 40). If no admin/maintainer access, skip the code-scanning API check but still check for other approaches. Code scanning check: run `gh api /repos/{owner}/{repo}/code-scanning/alerts` for SAST tools (Semgrep, CodeQL, Snyk); 403 \"Code Security must be enab",
  },
  {
    droidId: 'dependency_update_automation',
    piId: 'P4.5',
    ...
    skippable: false,
    description:
      'Dependabot, Renovate, or similar is configured and creating PRs for dependency updates. Keeps dependencies current automatically, reducing security vulnerability window.',
    evaluation:
  ...
  {
    droidId: 'gitignore_comprehensive',
    piId: 'P6.1',
    ...
    skippable: false,
    description:
      '.gitignore properly excludes .env files (not .env.example), node_modules, build artifacts, IDE configs (.idea, .vscode), and OS files (.DS_Store). Prevents agents from accidentally committing secrets or generated files.',
    evaluation:
  ...
  },
  {
    ...
    skippable: false,
    description:
      'Check for policies or tooling that enforce a minimum waiting period before adopting new dependency releases. Look for: 1) Renovate configured with `minimumReleaseAge` or `stabilityDays` (or an equivalent delay gate). 2) A documented dependency-update policy that explicitly requires waiting N days before merging version bumps. 3) Custom CI checks that verify the target release date is at least N days old. PASS only if there is an explicit delay (not just centralized updates or signature/provenanc',
    evaluation:
  ...
  },
  {
    ...
    skippable: false,
    description:
      '.github/ISSUE_TEMPLATE/ (GitHub) or .gitlab/issue_templates/ (GitLab) directory exists with structured templates for bugs, features, etc. Teaches agents what information to provide when creating issues.',
    evaluation:
  ...
  },
  {
    ...
    skippable: false,
    description:
      'Consistent labels exist for priority (P0-P3 or critical/high/medium/low), type (bug, feature, chore), and area (frontend, backend, infra). Enables agents to filter and prioritize work programmatically.',
    evaluation:
  ...
  },
  {
    ...
    skippable: false,
    description:
      '.github/pull_request_template.md (GitHub) or merge request templates (GitLab) exist with sections for description, testing done, and relevant context. Ensures agent PRs include necessary information for reviewers.',
    evaluation:
  ...
  },
  {
    ...
    scope: 'app',
    skippable: false,
    description: 'tsconfig.json with "strict": true for TS, mypy.ini or [tool.mypy] in pyproject.toml for Py.',
    evaluation:
  ...
  },
  {
    ...
    scope: 'app',
    skippable: false,
    description: 'Prettier (.prettierrc*) for TS, Black ([tool.black] in pyproject.toml) for Py.',
    evaluation: 'Formatter – Prettier (.prettierrc*) for TS, Black ([tool.black] in pyproject.toml) for Py.',
  },
  {
    droidId: 'pre_commit_hooks',
    piId: 'P4.3',
    name: 'Pre-commit hooks',
    level: 2,
    scope: 'app',
    skippable: false,
    description: 'Husky/lint-staged for TS, .pre-commit-config.yaml with ruff/black for Py.',
    evaluation: 'Pre-commit hooks – Husky/lint-staged for TS, .pre-commit-config.yaml with ruff/black for Py.',
  },
  {
    droidId: 'strict_typing',
    piId: 'P5.6',
    name: 'Strict typing enabled',
    level: 2,
    scope: 'app',
    skippable: true,
    description:
      'Project uses strict type checking. Common approaches: TypeScript tsconfig.json with "strict": true, Python mypy strict mode in mypy.ini or pyproject.toml, SonarQube/SonarCloud for TypeScript (has type-related rules that complement strict mode; verify it is not explicitly disabled in sonar properties). Other type checkers or strict mode configurations also satisfy this criterion. Some languages (Rust, Go) are typed by default. Reason about each application and skip if unclear.',
    evaluation:
  ...
  {
    droidId: 'duplicate_code_detection',
    piId: 'P5.9',
  ...
  },
  {
    ...
    skippable: true,
    description:
      'Check for N+1 query detection tooling. Look for: 1) bullet gem for Rails. 2) nplusone for Python/Django. 3) DataLoader pattern usage (graphql-dataloader). 4) ORM query logging with analysis. 5) Database query analysis in tests. 6) APM with slow query detection configured. PASS if any N+1 detection mechanism exists. Skip for apps without database/ORM usage (e.g., frontend-only apps, libraries, static sites).',
    evaluation:
  ...
  {
    droidId: 'heavy_dependency_detection',
    piId: 'P5.16',
  ...
  {
    droidId: 'unused_dependencies_detection',
    piId: 'P5.11',
  ...
  {
    droidId: 'unit_tests_exist',
    piId: 'P2.1',
    name: 'Unit tests present',
    level: 1,
    scope: 'app',
    skippable: false,
    description: '*.test.ts / __tests__/ (TS) or tests/test_*.py (Py).',
    evaluation: 'Unit tests present – *.test.ts / __tests__/ (TS) or tests/test_*.py (Py).',
  },
  {
    droidId: 'integration_tests_exist',
    piId: 'P2.7',
    name: 'Integration tests present',
    level: 3,
    scope: 'app',
    skippable: false,
    description: 'cypress/, playwright.config.ts (TS) or tests/integration/, Behave .feature files (Py).',
    evaluation:
      'Integration tests present – cypress/, playwright.config.ts (TS) or tests/integration/, Behave .feature files (Py).',
  },
  {
    droidId: 'unit_tests_runnable',
    piId: 'P2.12',
    name: 'Tests runnable locally',
    level: 2,
    scope: 'app',
    skippable: false,
    description:
      '"test": "vitest" (or Vitest) script in package.json (TS) or pytest runnable via tox/make test (Py). Actually run the command you find to see if the tests really are runnable (do not worry about whether they pass, just if they can be run). Use flags like --listTests (vitest) or --collect-only (pytest) to verify runnability without running the full suite, which can take hours. It is very important to use these flags to avoid waiting for the entire test suite to complete.',
    evaluation:
  ...
  {
    droidId: 'test_performance_tracking',
    piId: 'P2.11',
  ...
  {
    droidId: 'test_naming_conventions',
    piId: 'P2.8',
    ...
    skippable: false,
    description:
      'Check for consistent test file naming enforcement in any language. PASS if ANY ONE of the following exists: 1) JS/TS: Vitest testMatch/testRegex, Vitest include patterns, or Mocha test directory config. 2) Python: pytest naming conventions in pytest.ini or pyproject.toml (test_*.py pattern). 3) Go: *_test.go convention (built-in, check tests exist following pattern). 4) Java: Maven/Gradle test source directories with naming patterns. 5) Any test framework configured with explicit naming patterns',
    evaluation:
  ...
  },
  {
    ...
    skippable: false,
    description:
      'Check for test isolation enforcement in any language. PASS if ANY ONE of the following exists: 1) JS/TS: Vitest parallelization (not --runInBand), Vitest threads, or test sharding configured. 2) Python: pytest-xdist for parallel execution. 3) Go: `go test -parallel` or `t.Parallel()` usage. 4) Java: JUnit parallel execution config, or Maven/Gradle parallel test forks. 5) Database isolation patterns (transactions, test databases, factories, testcontainers). 6) Test randomization enabled (--random',
    evaluation:
      'Test isolation – Check for test isolation enforcement in any language. PASS if ANY ONE of the following exists: 1) JS/TS: Vitest parallelization (not --runInBand), Vitest threads, or test sharding configured. 2) Python: pytest-xdist for parallel execution. 3) Go: `go test -parallel` or `t.Parallel()` usage. 4) Java: JUnit parallel execution config, or Maven/Gradle parallel test forks. 5) Database isolation patterns (transactions, test databases, factories, testcontainers). 6) Test randomization enabled (--randomize, pytest-randomly). 7) Any test framework configured for parallel or isolated ex',
  },
  {
    droidId: 'interactive_qa_exists',
    piId: 'P8.7',
    name: 'Interactive QA documented',
    level: 2,
    scope: 'app',
    skippable: false,
    description:
      'A concrete, agent-followable path to bring the application to an interactive state and exercise it end-to-end must be documented, regardless of app type (web/browser UI, TUI/CLI, backend/API server, mobile app, desktop app, batch/worker). This criterion is documentation-only; do NOT actually run anything. Look for the QA/run path in AGENTS.md, README, CONTRIBUTING, package.json/Makefile/justfile scripts, skills (SKILL.md), docker-compose, .env.example, or e2e/QA harnesses (Playwright/Cypress for',
    evaluation:
      'Interactive QA documented – A concrete, agent-followable path to bring the application to an interactive state and exercise it end-to-end must be documented, regardless of app type (web/browser UI, TUI/CLI, backend/API server, mobile app, desktop app, batch/worker). This criterion is documentation-only; do NOT actually run anything. Look for the QA/run path in AGENTS.md, README, CONTRIBUTING, package.json/Makefile/justfile scripts, skills (SKILL.md), docker-compose, .env.example, or e2e/QA harnesses (Playwright/Cypress for web, a TUI test harness for CLIs, curl/httpie or an OpenAPI/health endp',
  },
  {
    droidId: 'interactive_qa_runnable',
    piId: null,
    name: 'Interactive QA runnable',
    level: 3,
    scope: 'app',
    skippable: false,
    description:
      'Building on the documented QA path (see interactive_qa_exists), an agent must be able to ACTUALLY run the application and interactively exercise it end-to-end, regardless of app type (web/browser UI, TUI/CLI, backend/API server, mobile app, desktop app, batch/worker). Step 1 (attempt): follow the documented path to bring the app to an interactive state – install deps, stand up required local services, load env vars, launch the app, and drive at least one meaningful interaction (load a page and c',
    evaluation:
      'Interactive QA runnable – Building on the documented QA path (see interactive_qa_exists), an agent must be able to ACTUALLY run the application and interactively exercise it end-to-end, regardless of app type (web/browser UI, TUI/CLI, backend/API server, mobile app, desktop app, batch/worker). Step 1 (attempt): follow the documented path to bring the app to an interactive state – install deps, stand up required local services, load env vars, launch the app, and drive at least one meaningful interaction (load a page and click, run a command and read output, hit an endpoint and get a 2xx, etc.).',
  },
  {
    droidId: 'api_schema_docs',
    piId: 'P0.9',
  ...
  },
  {
    ...
    skippable: true,
    description:
      'Schema definition files exist for databases (Prisma schema, TypeORM entities, SQLAlchemy models, raw SQL schemas). Agents need to understand the data model to make correct changes. Skip for apps without databases.',
    evaluation:
  ...
  },
  {
    ...
    skippable: false,
    description:
      'Check for logging library in dependencies: TS/JS (winston, pino, bunyan, log4js in package.json), Python (structlog, loguru, python-json-logger in requirements/pyproject.toml), or custom logger module (src/logger.*, lib/logging.*). PASS if any logging library is installed OR a dedicated logger module exists.',
    evaluation:
  ...
  {
    droidId: 'distributed_tracing',
    piId: 'P7.5',
    name: 'Check for trace ID or request ID propagation through the app',
    level: 3,
    scope: 'app',
    skippable: false,
    description:
  ...
  {
    droidId: 'metrics_collection',
    piId: 'P7.6',
    name: 'Check for metrics/telemetry instrumentation (Datadog, Axiom,',
    level: 3,
    scope: 'app',
    skippable: false,
    description:
  ...
  {
    droidId: 'code_quality_metrics',
    piId: 'P5.13',
    name: 'Code quality metrics tracked',
    level: 4,
    scope: 'app',
    skippable: true,
    description:
      "Coverage, complexity, and maintainability metrics are monitored. If `gh` or `glab` CLI is available and authenticated, first check admin access: GitHub: `gh api repos/{owner}/{repo} --jq '.permissions.admin'`, GitLab: `glab api projects/{id} --jq '.permissions.project_access.access_level'` (need >= 40). If no admin/maintainer access, skip the code-scanning API check but still check for other approaches. Code scanning check: run `gh api /repos/{owner}/{repo}/code-scanning/analyses`; 403 \"Code Sec",
    evaluation:
  ...
  {
    droidId: 'error_tracking_contextualized',
    piId: 'P7.7',
    ...
    skippable: false,
    description:
      'Sentry, Bugsnag, or Rollbar is configured with source maps, breadcrumbs, and user context. Agents can trace production errors back to specific code paths with full stack traces.',
    evaluation:
  ...
  },
  {
    ...
    skippable: false,
    description:
      'PagerDuty, OpsGenie, or custom alerting rules are defined. The system actively notifies when things go wrong rather than waiting for someone to notice. Prerequisite for incident response.',
    evaluation:
  ...
  {
    droidId: 'deployment_observability',
    piId: 'P7.11',
    ...
    skippable: false,
    description:
      'Look for pointers to monitoring dashboards (Datadog, Grafana, New Relic links in docs or code comments). Check for deploy notification integrations (Slack webhooks, deployment annotations in monitoring). PASS if documentation references where to check deploy impact, even if dashboards are hosted externally.',
    evaluation:
  ...
  },
  {
    ...
    skippable: true,
    description:
      'Check for health check endpoints and liveness/readiness probes. Look for: 1) `/health`, `/healthz`, `/ready`, `/live` endpoints in routes. 2) Kubernetes liveness/readiness probes in deployment manifests. 3) Health check libraries (terminus, lightship for Node.js, django-health-check). 4) Docker HEALTHCHECK instruction. 5) Load balancer health check configuration. PASS if any health check mechanism is implemented. Skip for non-deployed services (e.g., libraries, CLI tools, scripts, batch jobs).',
    evaluation:
  ...
  {
    droidId: 'profiling_instrumentation',
    piId: 'P7.13',
  ...
  },
  {
    ...
    skippable: true,
    description:
      'Check for Dynamic Application Security Testing (DAST) in CI/CD. Look for: 1) OWASP ZAP configured in CI workflows (zap-scan action, zap-baseline). 2) Burp Suite Enterprise or Burp CI integration. 3) Nuclei scanner configured. 4) Other DAST tools (Acunetix, Netsparker, StackHawk). 5) Custom security test suites that hit running endpoints. PASS if any DAST tool runs against a staging/test environment in CI. This is distinct from SAST (static analysis) - DAST tests the running application. Skip for',
    evaluation:
  ...
  {
    droidId: 'product_analytics_instrumentation',
    piId: 'P7.8',
    ...
    skippable: false,
    description:
      'Mixpanel, Amplitude, PostHog, Heap, or GA4 is instrumented in the application. Agents can see whether features are actually used and measure the impact of their changes on user behavior.',
    evaluation:
  ...
  {
    droidId: 'error_to_insight_pipeline',
    piId: 'P7.14',
...
// Lookup by pi check ID
export function getCriterionByPiId(piId: string): CriterionDef | undefined {
  return CRITERIA_REGISTRY.find((c) => c.piId === piId);
}

// Lookup by Droid criterion ID
export function getCriterionByDroidId(droidId: string): CriterionDef | undefined {
  return CRITERIA_REGISTRY.find((c) => c.droidId === droidId);
}

// Get all agent-only criteria (no pi equivalent)
export function getAgentOnlyCriteria(): CriterionDef[] {
  return CRITERIA_REGISTRY.filter((c) => c.piId === null);
}

// Get all pi-mapped criteria
export function getPiMappedCriteria(): CriterionDef[] {
  return CRITERIA_REGISTRY.filter((c) => c.piId !== null);
}
