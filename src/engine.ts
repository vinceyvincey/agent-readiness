...
// Remediation action map: check ID → actionable fix description.
// Exposed via getRemediationAction() for the readiness_check tool's per-check detail mode.
const REMEDIATION_ACTIONS: Record<string, string> = {
  ...
  'P1.4': 'Add MCP config (mcp.json with mcpServers key) or CLAUDE.md for agent context.',
  'P1.6': 'Add lifecycle hooks (.factory/hooks.json with valid hook entries).',
  ...
  'P4.3':
    'Add pre-commit hooks AND activate them. Set core.hooksPath, use husky (.husky/_/), or install pre-commit with .pre-commit-config.yaml.',
  ...
  'P4.9':
    'Add release automation: CD workflow (.github/workflows/release.yml) or semantic-release/changesets installed in devDependencies.',
  'P5.1':
...
export function getRemediationAction(checkId: string): string {
  return REMEDIATION_ACTIONS[checkId] || 'No mapped remediation';
}
...
export function runReadiness(
  ...
  const punchlist: PunchItem[] = findings
    ...
    .map((c) => ({
      ...
      action: getRemediationAction(c.id),
      evidence: c.evidence,
    }));
