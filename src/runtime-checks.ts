...
export function getRuntimeVerifications(root: string, lang: string, passingIds: Set<string>): RuntimeVerification[] {
  ...
  const toolOnPath = (name: string) => {
    try {
      const r = spawnSync(name, ['--version'], { encoding: 'utf8', timeout: 3000 });
      return r.status === 0 || (r.stderr && r.stderr.length > 0);
    } catch {
      return false;
    }
  };
...
`;
    ...
    if (passingIds.has('P2.2') && scripts.test) {
      if (deps.vitest) {
        verifications.push({
          checkId: 'P2.2',
          description: 'Verify vitest can list tests',
          command: ['node', '-e', vitestVerifyScript],
          timeoutMs: 30000,
        });
      } else if (deps.jest) {
        verifications.push({
          checkId: 'P2.2',
          description: 'Verify jest can list tests',
    ...
    if (passingIds.has('P5.1') && scripts.lint && nodeModulesExists) {
      verifications.push({
        checkId: 'P5.1',
        description: 'Verify linter runs',
    ...
    if (passingIds.has('P5.3') && fs.existsSync(path.join(root, 'tsconfig.json')) && nodeModulesExists) {
      verifications.push({
        checkId: 'P5.3',
        description: 'Verify type checker runs',
...
`;
    ...
    if (passingIds.has('P5.2') && nodeModulesExists) {
      if (deps.prettier) {
        verifications.push({
          checkId: 'P5.2',
          description: 'Verify prettier check',
    ...
    if (passingIds.has('P2.12') && scripts.test) {
      if (deps.vitest) {
        verifications.push({
          checkId: 'P2.12',
          description: 'Verify tests are runnable (vitest)',
          command: ['node', '-e', vitestVerifyScript],
          timeoutMs: 30000,
        });
      } else if (deps.jest) {
        verifications.push({
          checkId: 'P2.12',
          description: 'Verify tests are runnable (jest)',
          command: ['npx', 'jest', '--listTests'],
          timeoutMs: 30000,
        });
      }
    ...
    if (passingIds.has('P4.3')) {
      if (fs.existsSync(path.join(root, '.pre-commit-config.yaml')) && toolOnPath('pre-commit')) {
        verifications.push({
          checkId: 'P4.3',
          description: 'Verify pre-commit hooks run',
        ...
        verifications.push({
          checkId: 'P4.3',
          description: 'Verify lint-staged runs',
          command: ['npx', 'lint-staged', '--debug'],
          timeoutMs: 30000,
        });
      }
    }
    ...
    if (passingIds.has('P6.4') && hasPkg) {
      if (toolOnPath('gitleaks')) {
        verifications.push({
          checkId: 'P6.4',
          description: 'Verify gitleaks scan runs',
        ...
        verifications.push({
          checkId: 'P6.4',
          description: 'Verify npm audit runs',
          command: ['npm', 'audit', '--dry-run'],
          timeoutMs: 30000,
        });
      }
    }
    ...
    if (passingIds.has('P5.1')) {
      if (hasPyproject && /ruff/i.test(fs.readFileSync(pyproject, 'utf8'))) {
        ...
        verifications.push({
          checkId: 'P5.1',
          description: 'Verify flake8 runs',
          command: ['flake8', '.'],
          timeoutMs: 30000,
        });
      }
    ...
    if (passingIds.has('P2.12')) {
      verifications.push({
        checkId: 'P2.12',
        description: 'Verify tests are runnable (pytest)',
        command: ['pytest', '--collect-only'],
        timeoutMs: 30000,
      });
    }
    ...
    if (passingIds.has('P6.4') && toolOnPath('pip-audit')) {
      verifications.push({
        checkId: 'P6.4',
        description: 'Verify pip-audit runs',
        command: ['pip-audit', '--dry-run'],
        timeoutMs: 30000,
      });
    }
    ...
    if (passingIds.has('P5.8') && toolOnPath('vulture')) {
      verifications.push({
        checkId: 'P5.8',
        description: 'Verify vulture runs',
        command: ['vulture', '.'],
        timeoutMs: 30000,
      });
    }
    ...
    if (passingIds.has('P2.2') || passingIds.has('P2.12')) {
      ...
      if (passingIds.has('P2.2'))
        verifications.push({
          checkId: 'P2.2',
          description: 'Verify go test can list tests',
          command: cmd,
          timeoutMs: 30000,
        });
      if (passingIds.has('P2.12'))
        verifications.push({
          checkId: 'P2.12',
          description: 'Verify tests are runnable (go)',
          command: cmd,
          timeoutMs: 30000,
        });
    }
    if (passingIds.has('P5.3')) {
      verifications.push({
        checkId: 'P5.3',
        description: 'Verify go vet runs',
        command: ['go', 'vet', './...'],
        timeoutMs: 30000,
      });
    }
    ...
    if (passingIds.has('P6.4') && toolOnPath('govulncheck')) {
      verifications.push({
        checkId: 'P6.4',
        description: 'Verify govulncheck runs',
        command: ['govulncheck', './...'],
        timeoutMs: 30000,
      });
    }
    ...
    if (passingIds.has('P2.2') || passingIds.has('P2.12')) {
      ...
      if (passingIds.has('P2.2'))
        verifications.push({
          checkId: 'P2.2',
          description: 'Verify cargo test compiles',
          command: cmd,
          timeoutMs: 60000,
        });
      if (passingIds.has('P2.12'))
        verifications.push({
          checkId: 'P2.12',
          description: 'Verify tests are runnable (cargo)',
          command: cmd,
          timeoutMs: 60000,
        });
    }
    if (passingIds.has('P5.3')) {
      verifications.push({
        checkId: 'P5.3',
        description: 'Verify cargo check runs',
        command: ['cargo', 'check'],
        timeoutMs: 60000,
      });
    }
    ...
    if (passingIds.has('P6.4') && toolOnPath('cargo-audit')) {
      verifications.push({
        checkId: 'P6.4',
        description: 'Verify cargo audit runs',
        command: ['cargo', 'audit'],
        timeoutMs: 30000,
      });
    }
...
export function runRuntimeVerifications(root: string, verifications: RuntimeVerification[]): RuntimeResult[] {
