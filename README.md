...
## Status
Research + design committed. **M1-M17 done + verified.** M1-M5: skill, engine, extension+--fix+CI gate, history+trend, validation harness + E1/E2 evidence, dogfooding + distribution. M6: check-correctness bug fixes + git provenance. M7: anti-gaming guards, difficulty axis, droid harness surface checks. M8: monorepo app discovery + per-app scoring. M9: agentic remediation. M10: Droid comparison experiment, synthesized optimal prompts. M11: extracted Droid's actual system prompts from session traces, created 84-criteria mapping, added 20 new checks. M12: criterion description registry (84 entries), full descriptions in agent prompt, agent-only criteria section, hybrid scoring model (deterministic floor, agent ceiling). M13: side-by-side evaluation harness (pi vs Droid on same repos, criteria-by-criteria comparison, fix comparison). M14: 30 new deterministic checks (9 gh-CLI + 21 config-file), assessment/remediation prompt split, 77/84 criteria now deterministic (92%), only 7 truly agent-only. M15: enriched assessmentPromptFor with Droid-style 5-phase methodology (Scan, App Discovery, Evaluation, Validation, Scoring), full per-criterion descriptions + verification commands for 7 agent-only criteria. M16: 3 more deterministic grep checks (circuit_breakers, pii_handling, log_scrubbing) + runtime verification layer (`--verify` flag, actually runs commands to close presence≠signal gap) + unit_tests_runnable as deterministic check + Droid-compatible flat pass rate in report. 81/84 criteria now deterministic (96%), only 3 truly agent-only. M17: `/readiness-full` pi command (full hybrid: assess + fix + validate + delta), `fullHybridPromptFor()` combined prompt, upgraded existing commands with flag parsing (`--verify`, `--droid-scoring`, `--strict`), `readiness_check` tool gains `verify` param. Rubric v0.9.0.
...
- src/fix.ts - --fix remediation drafts (dry-run) + agentPromptFor (remediation) + assessmentPromptFor (5-phase Droid-style assessment, no file mods, full descriptions + verification commands for 7 agent-only criteria) + fullHybridPromptFor (combined assess+fix+validate+re-run prompt for /readiness-full)
...
- test/ - 10 test suites (engine, history, checks, deepening, monorepo, fix, criteria-registry, side-by-side, runtime-checks, html-report) — 324 assertions, all ALL PASS
...
### Extension (M3, M17)
- .pi/extensions/agent-readiness/index.ts - readiness_check tool (with verify param) + /readiness-report (with --verify, --droid-scoring, --strict flags) + /readiness-fix (with --verify flag) + /readiness-full (full hybrid: assess + fix + validate + delta)
