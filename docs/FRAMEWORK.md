...
## v0.6 enhancements (M12: criterion registry + hybrid architecture)
- **Criterion description registry:** `src/criteria-registry.ts` maps all 84 Droid criteria with full descriptions, evaluation instructions, and pi check ID mappings. 47 criteria are mapped to pi checks; 36 are agent-only (require runtime verification, API access, or code analysis).
- **Full descriptions in agent prompt:** `agentPromptFor()` now includes the criterion **Description** and **Evaluation** fields for each failing check (sourced from Droid's actual session traces), not just a one-line action. This gives the agent the same context Droid's `/readiness-fix` prompt provides.
- **Agent-only criteria section:** The prompt now lists all 36 agent-only criteria with their descriptions and evaluation instructions, so the agent can evaluate criteria the deterministic engine can't check (e.g., branch protection, CI feedback time, interactive QA).
- **Hybrid scoring model (deterministic floor, agent ceiling):** The prompt explicitly instructs the agent to verify deterministic findings behaviorally, discover agent-only criteria, and augment the score. The deterministic score is the floor; the agent can raise it (false positives) or lower it (decorative configs).
- **Lookup functions:** `getCriterionByPiId()`, `getCriterionByDroidId()`, `getAgentOnlyCriteria()`, `getPiMappedCriteria()` exported from the registry.